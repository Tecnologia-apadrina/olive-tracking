...
const mapParcelRecord = (p, sigpacs = []) => ({
  id: p.id,
  name: p.name || '',
...
const normalizeSigpacRecord = (raw, detailsMap = null) => {
  ...
  const municipio = detail?.municipio
    || raw.sigpac_municipio
    || raw.municipio
    || '';
  const poligono = detail?.poligono
    || raw.sigpac_poligono
    || raw.poligono
    || '';
  const parcela = detail?.parcela
    || raw.sigpac_parcela
    || raw.parcela
    || '';
  const recinto = detail?.recinto
    || raw.sigpac_recinto
    || raw.recinto
    || '';
  return {
...
const fetchSigpacsForParcels = async (session, parcels) => {
  ...
  const normalizeFromParcelRecords = async (rawList) => {
    ...
    for (const raw of arr) {
      if (!raw) continue;
      const municipio = raw.sigpac_municipio || raw.municipio || '';
      const poligono = raw.sigpac_poligono || raw.poligono || '';
      const parcela = raw.sigpac_parcela || raw.parcela || '';
      const recinto = raw.sigpac_recinto || raw.recinto || '';
  ...
  const tryGenericModel = async (modelName) => {
    try {
      const records = await executeKw(
        ...
        [sigpacIds],
        {
          fields: ['id', 'municipio', 'poligono', 'parcela', 'recinto', 'sigpac_municipio', 'sigpac_poligono', 'sigpac_parcela', 'sigpac_recinto'],
        }
      );
      ...
      return arr
        .map((rec) => {
          ...
          return {
            id,
            parcel_id: parcelId,
            municipio: rec.sigpac_municipio || rec.municipio || '',
            poligono: rec.sigpac_poligono || rec.poligono || '',
            parcela: rec.sigpac_parcela || rec.parcela || '',
            recinto: rec.sigpac_recinto || rec.recinto || '',
          };
        })
...
const upsertOdooParcelLocal = async (parcel, countryCode) => {
  ...
  await db.public.none(
    `INSERT INTO odoo_parcelas(id, name, common_name, company, contract_percentage, notes, country_code, updated_at)
     ...
     ON CONFLICT (id, country_code) DO UPDATE SET
       ...
       updated_at = now()`,
    [id, name, common_name, company, contract_percentage, notes, countryCode]
  );
...
// Asegura que existe una fila en parcelas con el id de Odoo (sin sobrescribir campos existentes)
const ensureParcelaStub = async (parcel, countryCode) => {
  ...
  await db.public.none(
    ...
    [id, name || null, common_name || null, contract_percentage != null ? contract_percentage : null, countryCode]
  );
  return true;
};
...
router.get('/odoo/parcelas', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const parcels = await executeKw(
      ...
      [domain],
      {
        fields: ['id', 'name', 'common_name', 'company_id', 'contract_percentage', 'notes', 'sigpac_ids'],
        limit,
      }
    );
    const sigpacsByParcel = await fetchSigpacsForParcels(session, parcels);
    const items = Array.isArray(parcels)
      ? parcels.map((p) => mapParcelRecord(p, sigpacsByParcel.get(p.id) || []))
      : [];
  ...
  try {
    ...
    for (const parcel of items) {
      try {
        ...
        synced += 1;
        const created = await ensureParcelaStub(parcel, country);
        if (created) ensuredParcelas += 1;
      } catch (_) {
...
router.get('/odoo/parcelas/local', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const parcels = await db.public.many(
      `SELECT id, name, common_name, company, contract_percentage, notes, updated_at
         FROM odoo_parcelas
